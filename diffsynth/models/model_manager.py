...
class ModelManager:
    ...
    def load_model(self, file_path, model_names=None, device=None, torch_dtype=None):
        ...
        if isinstance(file_path, list):
            ...
            # extract all safetensors files from the index
            tensor_files = index_data["weight_map"].values()
            state_dict = {}
            for tensor_file in set(tensor_files):
                sub_file_path = os.path.join(model_dir, tensor_file)
                state_dict.update(load_file(sub_file_path))
